...
class AlphaPortfolioData(Dataset):
    ...
    def _save_sequences_to_cache(self):
        ...
        try:
            ...
            cache_data = {
                ...
                'episode_dates': self.episode_dates,  # Save episode dates
                'metadata': {
                    ...
                    'episode_dates_shape': self.episode_dates.shape if hasattr(self.episode_dates, 'shape') else None,
            ...
            logging.info(f"Sequence shapes - Sequences: {self.sequences.shape}, "
                    ...
                    f"Episode dates: {self.episode_dates.shape if hasattr(self.episode_dates, 'shape') else 'None'}")
    ...
    def get_dates_for_episode(self, idx):
        """Return the dates for a specific episode."""
        if hasattr(self, 'episode_dates') and len(self.episode_dates) > idx:
            return self.episode_dates[idx]
        else:
            return [f"t={i+1}" for i in range(self.T)]  # Fallback to generic time steps
    ...
    def _load_cached_sequences(self):
        ...
        try:
            ...
            if 'episode_dates' in cache_data:
                self.episode_dates = cache_data['episode_dates']
            else:
                logging.warning("No episode dates found in cache. Will need to be recreated.")
                self.episode_dates = []
            ...
            logging.info(f"Loaded shapes - Sequences: {self.sequences.shape}, "
                    f"Future returns: {self.future_returns.shape}, "
                    f"Masks: {self.masks.shape}, "
                    f"Episode dates: {getattr(self.episode_dates, 'shape', 'None')}")
    ...
    def _create_sequences(self):
        """Create sequences from raw data - optimized version with date tracking."""
        ...
        episodes_fwd = []
        episodes_masks = []
        self.episode_dates = []  # To store dates for each episode
        ...
        for start_idx in tqdm(range(num_episodes), desc="Creating episodes"):
            ...
            episode_fwd = []
            episode_masks = []
            episode_dates = []  # Track dates for this episode
            ...
            for t in range(self.T):
                ...
                # Store the future date for this time step
                episode_dates.append(future_date)
                
                # Process all assets efficiently
                for permno, permno_data in permno_grouped_data.items():
                    idx = self.permno_to_idx[permno]
                    
                    # Use date_indices for faster date filtering
                    ...
                    fwd_mask = permno_data['date'] == future_date
                    fwd_data = permno_data[fwd_mask]
            ...
            if len(episode_states) == self.T:
                ...
                self.episode_dates.append(episode_dates)
        ...
        self.episode_dates = np.array(self.episode_dates)  # Keep dates as numpy array
        ...
        logging.info(f"  Episode dates: {self.episode_dates.shape if hasattr(self.episode_dates, 'shape') else 'empty'}")
    ...
    def __getitem__(self, idx):
        """Return sequence, future returns, masks, and dates for an episode."""
        # Basic data
        sequence_data = self.sequences[idx]
        future_returns_data = self.future_returns[idx]
        masks_data = self.masks[idx]
        
        # Date data (if available)
        episode_dates = None
        if hasattr(self, 'episode_dates') and len(self.episode_dates) > idx:
            episode_dates = self.episode_dates[idx]  # Keep as original format
        
        # Return all data
        if episode_dates is not None:
            return sequence_data, future_returns_data, masks_data, episode_dates
        else:
            return sequence_data, future_returns_data, masks_data
...
def custom_collate_fn(batch):
    """Custom collate function that properly handles date information."""
    # Check if batch includes dates (4 items per batch element)
    has_dates = len(batch[0]) == 4
    
    if has_dates:
        # Separate regular tensors from dates
        sequences = [item[0] for item in batch]
        future_returns = [item[1] for item in batch]
        masks = [item[2] for item in batch]
        dates = [item[3] for item in batch]  # Keep dates as Python objects
        
        # Use default collate for tensor data
        collated_sequences = torch.utils.data.dataloader.default_collate(sequences)
        collated_future_returns = torch.utils.data.dataloader.default_collate(future_returns)
        collated_masks = torch.utils.data.dataloader.default_collate(masks)
        
        # Return combined batch
        return collated_sequences, collated_future_returns, collated_masks, dates
    else:
        # Standard processing for batches without dates
