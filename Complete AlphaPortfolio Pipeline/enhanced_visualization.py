...
class VisualizationManager:
    ...
    def visualize_episode(self, episode_idx: int, epoch: int, batch_idx: int, phase: str,
        ...
        Args:
            ...
            dates: Optional list of dates for this episode
        """
        ...
        # Plot episode returns (add dates parameter)
        ...
        # Plot episode portfolio allocation (add dates parameter)
        ...
        # Plot episode cumulative returns (add dates parameter)
        ...
        # Plot episode asset selection (add dates parameter)
    ...
    def _format_date_labels(self, dates):
        """Helper to format date labels for visualization."""
        if dates is None:
            return None
        ...
        for date in dates:
            try:
                ...
                formatted_dates.append(f"t={len(formatted_dates)+1}")
    ...
    def _plot_episode_returns(self, returns: np.ndarray, output_dir: str, 
        ...
        plt.plot(range(len(returns)), returns * 100, 'b-o')
    ...
    def _plot_episode_portfolio_allocation(self, weights: np.ndarray, output_dir: str, 
        ...
        # Format date labels for x-axis if provided
        x_labels = None
        if dates is not None and len(dates) == len(weights):
            ...
            x_labels = [f"t={i+1}" for i in range(len(weights))]
    ...
    def _plot_episode_asset_selection(self, weights: np.ndarray, 
        ...
        # Get the date label for the interesting step
        date_label = x_labels[interesting_step] if x_labels and interesting_step < len(x_labels) else f"t={interesting_step+1}"
        ...
        if len(long_indices) > 0:
            ...
            ax1.set_title(f'Top Long Positions ({date_label})', fontsize=12)
        ...
        if len(short_indices) > 0:
            ...
            ax2.set_title(f'Top Short Positions ({date_label})', fontsize=12)
        ...
        ax3.set_xlabel('Date' if dates is not None else 'Time Step', fontsize=10)
    ...
    def _plot_epoch_returns_distribution(self, returns: np.ndarray, output_dir: str, 
        ...
        # Check if returns array is empty
        if returns.size == 0 or returns.ndim == 0:
            # Handle empty arrays gracefully
            plt.text(0.5, 0.5, "No return data available for this epoch", 
        ...
        # Add key statistics as text - with safety checks
        stats_text = (
        ...
        # Only add min/max if the array is not empty
        ...
        # Add positive returns percentage if data exists
        if flat_returns.size > 0:
            stats_text += f"Positive Returns: {np.mean(flat_returns > 0) * 100:.1f}%"
